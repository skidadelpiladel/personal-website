    ...
    <div className="min-h-screen bg-[#0c0c0e] text-white flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-[420px]">
        ...
        <div className="bg-[#161618] border border-[#252529] rounded-[20px] p-7 md:p-8">
          <div className="w-10 h-10 rounded-xl bg-[#facc15] grid place-items-center text-[#0c0c0e] font-bold">◈</div>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <div>
              ...
              <input value={user} onChange={e=>setUser(e.target.value)} required autoComplete="username" className="mt-1 w-full bg-[#0c0c0e] border border-[#252529] rounded-xl px-3 py-2.5 text-sm focus:border-[#facc15] outline-none" placeholder="admin" />
            </div>
            <div>
              <label className="font-mono text-xs tracking-widest text-[#9f9fa3]">PASSWORD</label>
              <input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} required autoComplete="current-password" className="mt-1 w-full bg-[#0c0c0e] border border-[#252529] rounded-xl px-3 py-2.5 text-sm focus:border-[#facc15] outline-none" placeholder="••••••••" />
            </div>
            {err && <div className="bg-red-500/10 border border-red-500/20 text-red-300 text-xs px-3 py-2 rounded-xl">{err}</div>}
            <button disabled={loading} className="w-full bg-[#facc15] text-[#0c0c0e] font-semibold text-sm py-3 rounded-full hover:bg-[#fde047] disabled:opacity-60 transition-colors">
